...
def import_data(file_name, city_index, model):
    ...
    with open(path) as f:
            ...
            for line in data:
                    city = City.objects.filter(city__icontains=line[city_index])
                    if city:
                            print('Found city', city)
                            ob = model(city=city, state=line[1], walk_score=row[2],
...
import_data(w, 0, Walk)
**kwargs


'''
#run this with every table
def import_walk():
    cursor = setup_cursor()
    if cursor is None:
        print('no cursor')
